...
def create_projects_capacities(projects_list,max_capacity):
    projects_cap = {}
...
def start_instance0(minchoice,nbpersonnes,nbproject):
    min_choices = minchoice 
    sl = create_random_list(nbpersonnes, "Nom complet_")
    pl = create_random_list(nbproject, "project_")
    #pc = create_projects_capacities(pl, 6) # nombre de place par projet
    prefs = create_pref_dict(sl, pl, min_choices) 
